  ...
  return (
    <div className="p-4 bg-card text-card-foreground bg-slate-100" style={{ marginLeft: "256px" }}>
          ...
          <div className="bg-white p-6 rounded-md w-1/2">
            ...
            <div className="mb-4">
              ...
              <select
                ...
                className="mt-1 block w-full border border-input rounded-md shadow-sm"
              >
                <option value="" hidden>Chọn loại tài khoản</option>
            ...
            <div className="flex justify-end">
              <button onClick={closePopup} className="bg-gray-500 text-white px-4 py-2 rounded-md mr-2">Hủy</button>
              <button onClick={handleSubmit} className="bg-green-500 text-white px-4 py-2 rounded-md">Lưu</button>
            </div>
          </div>
        </div>
      )}
        ...
        <table className="min-w-full bg-white border border-border rounded-md">
          ...
          <tbody>
            {listData
              ...
              .map((data, index) => (
                <tr key={index}>
                  <td className="px-1 py-1 text-center border">{index + 1 + (currentPage - 1) * itemsPerPage}</td>
                  <td className="px-1 py-1 text-center border">{data.username}</td>
      ...
      <div className="flex items-center justify-between mt-4">
        <span className="text-sm">Hiện {Math.min((currentPage - 1) * itemsPerPage + 1, totalItems)} đến {Math.min(currentPage * itemsPerPage, totalItems)} trong {totalItems} mục</span>
